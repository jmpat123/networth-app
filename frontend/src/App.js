...
function App() {
  ...
  const fetchAllData = () => {
    ...
    }).catch(e => setError('Failed to fetch summary'));
  ...
  const handleManualAssetSubmit = async (e) => {
    ...
    } catch (err) { setError('Failed to add asset'); }
  };
  ...
  const handleEdit = (h) => {
    setForm({
        ...
        symbol: h.symbol,
        quantity: h.quantity,
        priceUsd: h.purchase_price_usd || h.price_usd, // Load Cost Basis
        assetClass: h.asset_class,
        effectiveDate: h.effective_date || new Date().toISOString().slice(0, 10)
    });
    // Scroll to top so user sees the form
    window.scrollTo({ top: 0, behavior: 'smooth' });
    setMessage(`Loaded ${h.symbol} into form. Make changes and click 'Add Asset'.`);
  };
  ...
  const handleLiabilitySubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => setMessage(''), 3000);
    } catch (err) { setError('Failed to add liability'); }
  };

  const handleWalletSubmit = async (e) => {
    e.preventDefault();
    try {
      setMessage('Syncing Wallet... this may take a moment.');
      await fetch(`${backendUrl}/api/wallets/add`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(walletForm)});
      await fetch(`${backendUrl}/api/wallets/refresh`); 
      await fetch(`${backendUrl}/api/snapshots/write`, { method: 'POST' });
      fetchAllData();
      setMessage('Wallet Added & Syncing Complete');
      setTimeout(() => setMessage(''), 3000);
      setWalletForm({ address: '', chain: 'eth', nickname: '' });
    } catch (err) { setError('Wallet sync failed'); }
  };
  ...
  const handlePriceRefresh = async () => {
    ...
    try {
      ...
      setTimeout(() => setMessage(''), 3000);
    } catch (err) { setError('Price refresh failed'); }
  };
  ...
  const { open, ready } = usePlaidLink({
    ...
    onSuccess: async (public_token) => {
        try {
          ...
          await fetch(`${backendUrl}/api/plaid/sync-holdings`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({connectionId: 'TODO'})});
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'system-ui, sans-serif', background: '#f4f4f5' }}>
        ...
        <div style={{ width: '280px', background: '#fff', borderRight: '1px solid #e4e4e7', padding: '24px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
            ...
            <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <button onClick={() => ready ? open() : null} disabled={!ready || plaidSyncing} style={btnStyle}>
                        {plaidSyncing ? 'Syncing Plaid...' : '+ Connect Plaid'}
                    </button>
                    ...
                    <div style={cardStyle}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <input placeholder="Qty" type="number" value={form.quantity} onChange={e => setForm({...form, quantity: e.target.value})} style={inputStyle} />
                            <input placeholder="Cost Basis ($)" type="number" value={form.priceUsd} onChange={e => setForm({...form, priceUsd: e.target.value})} style={inputStyle} />
                        </div>
        ...
        <div style={{ flex: 1, padding: '32px', overflowY: 'auto' }}>
            ...
            {/* NOTIFICATIONS */}
            {message && <div style={{padding: '12px', background: '#dcfce7', color: '#166534', borderRadius: '8px', marginBottom: '24px', fontWeight: '500'}}>✓ {message}</div>}
            {error && <div style={{padding: '12px', background: '#fee2e2', color: '#991b1b', borderRadius: '8px', marginBottom: '24px', fontWeight: '500'}}>⚠ {error}</div>}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
                ...
                {/* HOLDINGS TABLE (Updated Grouping + Gains + Edit) */}
                <div style={bigCardStyle}>
                    <h3 style={cardHeaderStyle}>Holdings</h3>
                    
                    {holdings.length === 0 ? (
                        <p style={{color: '#aaa', fontStyle: 'italic'}}>No holdings yet.</p>
                    ) : (
                        <div>
                            {/* GROUPING LOGIC */}
                            {Object.values(holdings.reduce((groups, h) => {
                                const connId = h.accounts?.connections?.id || 'unknown';
                                if (!groups[connId]) {
                                    const provider = h.accounts?.connections?.provider;
                                    let groupName = 'Unknown';
                                    if (provider === 'manual') groupName = '📝 Manual Portfolio';
                                    else if (provider === 'wallet') groupName = `🦊 Crypto Wallet (${h.accounts?.connections?.nickname || 'Main'})`;
                                    else if (provider === 'plaid') groupName = `🏦 ${h.accounts?.name || 'Bank'} (Plaid)`;
                                    groups[connId] = { id: connId, name: groupName, items: [] };
                                }
                                groups[connId].items.push(h);
                                return groups;
                            }, {})).map(group => (
                                <div key={group.id} style={{ marginBottom: '24px' }}>
                                    {/* GROUP HEADER */}
                                    <h4 style={{ margin: '0 0 12px 0', fontSize: '13px', textTransform: 'uppercase', color: '#a1a1aa', borderBottom: '1px solid #f4f4f5', paddingBottom: '4px' }}>
                                        {group.name}
                                    </h4>

                                    <table style={{ width: '100%', fontSize: '14px', borderCollapse: 'collapse' }}>
                                        <thead style={{ color: '#71717a', borderBottom: '1px solid #e4e4e7' }}>
                                            <tr>
                                        ...
                                        <tbody>
                                            {group.items.map(h => {
                                                ...
                                                const currentPrice = Number(h.price_usd);
                                                const costPrice = Number(h.purchase_price_usd) || currentPrice;
                                                ...
                                                return (
                                                    <tr key={h.id} style={{ borderBottom: '1px solid #f4f4f5' }}>
                                                        <td style={tdStyle}>
                                                            <b>{h.symbol}</b>
                                                            <span style={{ color: '#a1a1aa', fontSize: '11px', marginLeft: '6px', background: '#f4f4f5', padding: '2px 4px', borderRadius: '4px' }}>
                                                                {h.asset_class}
                                                            </span>
                                                        </td>
                                                        <td style={tdStyle}>{qty.toLocaleString()}</td>
                                                        <td style={{ ...tdStyle, color: '#71717a' }}>${costPrice.toLocaleString()}</td>
                                                        <td style={tdStyle}>${currentPrice.toLocaleString()}</td>
                                                        <td style={{ ...tdStyle, fontWeight: '600' }}>${totalValue.toLocaleString()}</td>
                                                        
                                                        {/* GAIN LOSS COLUMN */}
                                                        <td style={{ ...tdStyle, color: isBreakEven ? '#71717a' : (isUp ? '#16a34a' : '#dc2626'), fontWeight: '500' }}>
                                                            {!isBreakEven ? (
                                                                <>
                                                                    {isUp ? '+' : ''}${Math.abs(gainLoss).toLocaleString()} <br/>
                                                                    <span style={{fontSize:'11px'}}>({isUp ? '+' : ''}{gainLossPct.toFixed(2)}%)</span>
                                                                </>
                                                            ) : '—'}
                                                        </td>
                                                        ...
                                                        <td style={tdStyle}>
                                                            ...
                                                            <button 
                                                              ...
                                                              style={{ color: '#2563eb', border: 'none', background: 'none', cursor: 'pointer', fontWeight: 'bold', marginRight: '8px' }}
